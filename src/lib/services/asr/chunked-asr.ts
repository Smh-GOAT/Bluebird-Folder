...
async function transcribeWithRetry(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const result = await transcribeByQwen3Asr(chunkPath);
      return result;
    } catch (error) {
...
export async function transcribeAudioChunks(
  ...
  try {
    ...
    for (let i = 0; i < chunkPaths.length; i++) {
      ...
      const result = await transcribeWithRetry(chunkPath, i, maxAttempts, onLog);
...
export async function transcribeChunked(
  audioPath: string,
  sliceOptions: { onLog?: (message: string) => void; maxAttempts?: number } = {}
): Promise<ChunkedAsrResult> {
  const { sliceAudio } = await import("@/lib/services/audio");
  
  const { onLog, maxAttempts = 2 } = sliceOptions;
  ...
  return transcribeAudioChunks(chunkPaths, { onLog, maxAttempts });
}
