...
export function GenerateStatus({ error, onRetry }: GenerateStatusProps) {
  if (error) {
    return (
      <section className="ui-panel-elevated flex h-full min-h-[560px] flex-col items-center justify-center p-6 text-center lg:min-h-0">
        ...
        <h3 className="mt-4 text-lg font-semibold" style={{ color: "var(--text)" }}>生成失败</h3>
        <p className="mt-2 max-w-sm text-sm" style={{ color: "var(--text-muted)" }}>{error}</p>
        <button type="button" onClick={onRetry} className="ui-btn-primary mt-6 px-6 py-2.5">
          重试
        </button>
      </section>
    );
  ...
  return (
    <section className="ui-panel-elevated flex h-full min-h-[560px] flex-col items-center justify-center p-6 text-center lg:min-h-0">
      ...
      <h3 className="mt-4 text-lg font-semibold" style={{ color: "var(--text)" }}>正在生成总结...</h3>
